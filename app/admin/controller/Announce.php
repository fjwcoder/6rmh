...
class Announce extends Manage
{
    ...
    {   
        ...
        $list = db('announce', [], false) ->order('id desc') -> paginate();
    ...
    public function edit(){
        ...
        $result = db('announce', [], false) -> where(array('id'=>$id)) -> find();
    ...
    public function del(){
        ...
        $rs = db('announce')->where(array('id'=>$id))->delete();
        if ($rs) {
            $this->success('删除成功', "Announce/index");
        } else {
            $this->error('删除失败');
        }
    }
    ...
    public function dataPost($type=''){
        ...
        if($type=='add'){
            // return dump($data);
            $result = db('announce', [], false) -> insert($data);
        }else{
            $id = $data['id'];
            unset($data['id']);
            unset($data['addtime']);
            $result = db('announce', [], false) -> where(array('id'=>$id)) ->update($data);
        }

        
        if($result){
            // return $this->success('成功', "request()->controller()/index");
            return $this->success('成功', "Announce/index");
        }else{
