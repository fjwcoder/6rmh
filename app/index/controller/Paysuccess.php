...
class Paysuccess extends controller
{
    ...
    public function order($resArr, $order=[]){
        if(!empty($order)){
            ...
            Db::name('payresult_step') -> insert(['order_id'=>$order['order_id'], 'content'=>'本用户计算完成', 'step'=>4]);
            ...
            Db::name('payresult_step') -> insert(['order_id'=>$order['order_id'], 'content'=>'订单状态修改完成', 'step'=>7]);
            if($update){
                $log_bait = Db::name('bait_log') -> insertAll($bait_log);

                $log_point = Db::name('point_log') -> insertAll($point_log);
                Db::name('payresult_step') -> insert(['order_id'=>$order['order_id'], 'content'=>'日志写入完成', 'step'=>8]);
                return true;
            }else{
                return false;
            }
