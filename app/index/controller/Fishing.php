            ...
            if(empty($user)){
                ...
                if( ($user['bait']<1) || ($user['point']<$mallConf['FISH_USE_POINT']['value'] ) ){
                    ...
                    if($rand<=$mallConf['FISH_POINT_PERCENT']['value']){//积分
                        $get_point = Db::name('users') -> where(['id'=>$uid, 'status'=>1]) -> setInc('point', $mallConf['FISH_GET_POINT']['value']);
                        if($get_point){
                            Db::name('point_log') -> insert(['userid'=>$uid, 'name'=>$user['nickname'], 
                            'value'=>$mallConf['FISH_GET_POINT']['value'], 'type'=>1, 'remark'=>'【'.$user['nickname'].'】钓鱼获得【'. $mallConf['FISH_GET_POINT']['value'].'】积分']);
                        }
                        ...
                        if($rand<=$mallConf['FISH_MOON_PERCENT']['value']){ //特殊物品 => 月亮
                            ...
                            echo json_encode(array('status'=>true, 'content'=>'获得红包【'.$money['money'].'】元'), JSON_UNESCAPED_UNICODE);
                            // return '获得红包';
