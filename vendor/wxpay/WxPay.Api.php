...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder_fjw($inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
		//检测必填参数
		if(!$inputObj->IsOut_trade_noSet()) {
	...
	public static function redPack($inputObj, $timeOut = 6){
		...
		if(!$inputObj->IsMch_billnoSet()) {
			throw new WxPayException("缺少统一支付接口必填参数mch_billno！");
		}else if(!$inputObj->IsSend_nameSet()){
			throw new WxPayException("缺少统一支付接口必填参数send_name！");
		}else if(!$inputObj->IsRe_openidSet()) {
			throw new WxPayException("缺少统一支付接口必填参数re_openid！");
		}else if(!$inputObj->IsTotal_amountSet()) {
			throw new WxPayException("缺少统一支付接口必填参数total_amount！");
		}else if(!$inputObj->IsTotal_numSet()){
			throw new WxPayException("缺少统一支付接口必填参数total_num！");
		}else if(!$inputObj->IsWishingSet()) {
			throw new WxPayException("缺少统一支付接口必填参数wishing！");
		}else if(!$inputObj->IsAct_nameSet()) {
			throw new WxPayException("缺少统一支付接口必填参数act_name！");
		}else if(!$inputObj->IsRemarkSet()) {
			throw new WxPayException("缺少统一支付接口必填参数remark！");
		}
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function businessPay($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsPartner_trade_noSet()) {
			...
			throw new WxPayException("缺少统一支付接口必填参数check_name！");
		}else if(!$inputObj->IsRe_user_nameSet()) {
	...
	public static function orderQuery($inputObj, $timeOut = 6)
	{
		...
		/* $logFile = "E:/PHP_Develop/5FZ2/App/Mobile/Controller/after.txt";
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_CAINFO, WxPayConfig::CAINFO_PATH);
		}
		//post提交方式
